...
description: Overview of the AI architecture, including role-based configuration and observability metadata.
icon: 🚀
order: 10
---
...
4. **Reviewer (Critique)**: Validates output against strict rules.

## Default Models (Configurable via Environment Variables)

The CLI automatically selects appropriate models based on the detected API key provider. Please ensure that the API keys mentioned below are valid and supported:

- **OpenAI API Key (`OPENAI_API_KEY`)**:
    - **Planner**: `gpt-4` (Reasoning).
    - **Writer**: `gpt-4` (Fast generation).
    - **Researcher**: `gpt-4` (Fast context gathering).
...
| Command             | Planner (e.g. `gpt-4`)                                                                                                                                                         | Researcher (e.g. `gpt-4`)                                                                                                                                  | Writer (e.g. `gpt-4`)                                                                                          | Reviewer (e.g. `gpt-4`)                                                                                              | Main AI Input                                                                            |
| :------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
...
The architecture now supports observability metadata for enhanced AI tracking. Each AI agent can log its actions and decisions, providing insights into the AI's performance and behavior. This is facilitated through the `ObservabilityMetadata` parameter in the `generateText` method, allowing for detailed tracking of AI interactions.

### Example of Logging with Observability Metadata

When generating text, you can include metadata to track the agent's role and any relevant tags:

```ts
const text = await writerAgent.generateText(
    'Generate an overview of the project.',
    {
        temperature: 0.7,
        maxTokens: 150,
    },
...
This metadata can be used for performance analysis and debugging, ensuring that the AI's contributions are transparent and traceable.
