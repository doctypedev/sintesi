...
| `--base`          |         | string  | Lineage last-generated SHA (from `.sintesi/lineage.json`) if present | Explicit base ref/branch/SHA to use for comparison (overrides cached/shared lineage state). If not provided, Sintesi uses the LineageService last-generated SHA (from `.sintesi/lineage.json`) as the baseline for comparison. Note: some earlier CLI help text referenced `origin/main` as a default — that is inaccurate; the runtime default is the lineage SHA when available. |
...
- Baseline resolution: by default Sintesi now uses the LineageService last-generated SHA (from `.sintesi/lineage.json`) as the baseline for diff/analysis. Use `--base` to explicitly override and point the check at another commit or branch. Sintesi no longer falls back to local ephemeral state files such as `.sintesi/*.state.json` as a baseline; those files are intentionally excluded from CI caches to avoid restoring ephemeral local state across runs.
- If no baseline can be determined (no lineage SHA found and no `--base` provided), the command logs warnings and returns failure (`success: false`, `configError`). The CLI wrapper will exit with code 1 in this case when `--strict` is enabled.
