...
import { DOC_PLANNING_PROMPT, DOC_DISCOVERY_PROMPT } from '../prompts/documentation';
...
export class DocumentationPlanner {
    ...
    async createPlan(
        ...
        // Create observability metadata for tracking this documentation generation session
        ...
        // --- DISCOVERY STEP (Architecture Analysis) ---
        // If we have a Researcher Agent, let it scan the "Forest" before the Planner scans the "Trees".
        ...
        if (aiAgents.researcher) {
            this.logger.info('🕵️‍♂️ Researcher performing architectural discovery...');
            let readmeContent = '';
            try {
            ...
            try {
                const discoveryPrompt = DOC_DISCOVERY_PROMPT(
                    packageJsonSummary,
                    fileSummary,
                    readmeContent || existingDocsSummary,
                );
                const architecturalBrief = await aiAgents.researcher.generateText(
                    discoveryPrompt,
                    {
                        maxTokens: 2000,
                        temperature: 0.2,
                    },
                    {
                        ...
                        properties: {
                            ...
                            feature: 'architectural-discovery',
                        },
                        tags: [...(sessionMetadata.tags || []), 'discovery', 'architecture'],
                    },
                );
                specificContext += `\n\n## 🏛️ ARCHITECTURAL BRIEF (By Researcher Agent)\nThis brief was generated by analyzing the project structure. Use it to guide your plan.\n\n${architecturalBrief}\n\n`;
                this.logger.success('✅ Architectural Brief generated.');
                this.logger.debug(architecturalBrief);
            } catch (e) {
                this.logger.warn('Discovery step failed: ' + e);
            }
        }
...
`;
        ...
        try {
            let response = await aiAgents.planner.generateText(
                planPrompt,
                {
                    maxTokens: 2000,
                    temperature: 0.1,
