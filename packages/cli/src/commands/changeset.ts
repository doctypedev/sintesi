 ...
 */
export async function changesetCommand(options: ChangesetOptions = {}): Promise<ChangesetResult> {
    const {
        baseBranch = 'main',
        stagedOnly = false,
    ...
    try {
        ...
        logger.debug(
            `Packages: ${resolvedPackageNames.join(', ')}${
                packageName ? ' (manual)' : selection.automatic ? ' (auto-detected)' : ' (selected)'
            }`,
        );
        ...
        const relevantPackagePaths = resolvedPackageNames
            .map((name) => {
        ...
        if (relevantPackagePaths.length > 0 && monorepoInfo.isMonorepo) {
            ...
            analysis.symbolChanges = analysis.symbolChanges.filter((change) => {
                ...
                return relevantPackagePaths.some(
                    (pkgPath) =>
                        change.filePath.startsWith(pkgPath + path.sep) ||
                        change.filePath === pkgPath,
                );
            });
            ...
            analysis.changedFiles = analysis.changedFiles.filter((file) => {
                return relevantPackagePaths.some(
                    (pkgPath) => file.startsWith(pkgPath + path.sep) || file === pkgPath,
                );
            });
            ...
            logger.debug(
                `Filtered analysis to ${analysis.totalChanges} changes for selected packages`,
            );
        }
