...
export class VersionTypeDetector {
    ...
    detect(analysis: ChangesetAnalysis): VersionDetectionResult {
        ...
        this.logger.debug(
            `Changes: ${added.length} added, ${deleted.length} deleted, ${modified.length} modified`,
        );
    ...
    private buildDetailedDescription(changes: {
        ...
        if (changes.breakingModifications && changes.breakingModifications.length > 0) {
            const symbols = changes.breakingModifications
                .map((c) => `\`${c.symbolName}\``)
                .join(', ');
            if (changes.breakingModifications.length === 1) {
                ...
                parts.push(
                    `${prefix}Change signatures of ${changes.breakingModifications.length} symbols: ${symbols}`,
                );
            }
        }
        ...
        if (changes.featureModifications && changes.featureModifications.length > 0) {
            const symbols = changes.featureModifications
                .map((c) => `\`${c.symbolName}\``)
                .join(', ');
        ...
        if (changes.modified && changes.modified.length > 0) {
            const symbols = changes.modified
                .slice(0, 5)
                .map((c) => `\`${c.symbolName}\``)
                .join(', ');
            const more =
                changes.modified.length > 5 ? ` and ${changes.modified.length - 5} more` : '';
